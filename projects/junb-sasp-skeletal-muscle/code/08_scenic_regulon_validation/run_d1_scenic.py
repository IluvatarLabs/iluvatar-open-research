...
# Force multiprocessing to use fork (not spawn) before any other imports
# This is critical for dask/arboreto compatibility
...
# Force arboreto to NOT use dask (avoids multiprocessing spawn issues)
...
def prepare_expression_for_grnboost(adata, min_cells=0.01):
    ...
    Returns: ex_matrix (cells × genes DataFrame with HGNC symbols)
    """
...
for ds_name, ds_info in datasets.items():
    ...
    print(f"    Running GRNBoost2 on {ex_snrna.shape[0]} cells × {ex_snrna.shape[1]} genes...")
    print(f"    Target TFs for inference: {len(tf_in_data)}")

    t_grn_start = time.time()

    # Use only target TFs for faster inference (instead of all human TFs)
    # This reduces runtime from hours to ~1 hour
    ...
    print(f"    Running GRNBoost2 on {ex_all.shape[0]} cells × {ex_all.shape[1]} genes...")
    ...
    print(f"    Running AUCell on {ex_all_for_aucell.shape[0]} cells × {len(regulon_genesets)} regulons...")
